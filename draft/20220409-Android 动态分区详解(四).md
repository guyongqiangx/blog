...
在 Android Q 引入动态分区之前的版本，包括 Android N, O, P 的 Manifest 都是一样的，但从 Android Q 开始，在 payload 的 manifest 中引入了描述动态分区的数据 `dynamic_partition_metadata`。

我以前分析 Android N 的 update_engine 代码时画过一张图，现在将这张图针对动态分区内容进行更新，得到 manifest 新版本的结构信息，如下：
