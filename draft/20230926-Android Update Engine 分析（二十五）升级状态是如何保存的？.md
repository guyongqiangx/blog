...
我们以基于文件存储数据的 Prefs 类的 SetInt64() 函数的代码来看看具体的实现。
...
打比方说，这有点像一个部长(`daemon_state_`)，负责整个部门的运作，部门下面有多个子部门(`boot_control_`， `hardware_`, `prefs_` 等)，具体的工作都交由常务副部长(`update_attempter_`)去管理。但是，还存在一个专职秘书(`certificate_checker_`)，单独对部长负责。
