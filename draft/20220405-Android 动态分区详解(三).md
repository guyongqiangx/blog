...
动态分区虽然有两套参数，但最终这两套参数会合二为一成为一套参数，并将这些参数设置输出到 `misc_info.txt` 中。两套参数的处理请参考本文的第 3 节。

编译系统调用 `build_super_image.py` 脚本读取 `misc_info.txt`中的动态分区配置参数，传递给 lpmake 工具。lpmake 根据动态分区参数中各分区的大小以及 image 路径，生成最终的 super.img。
