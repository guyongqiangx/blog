...
上一篇[《Android AVB 分析（十一）bootloader 是如何进行 verify boot 检查的？》](https://blog.csdn.net/guyongqiangx/article/details/144995975)分析了 bootloader 中是如何调用 libavb 函数库验证 vbmeta 和 boot 等分区的。

接下来就打算写一篇介绍 dm-verity 原理的文章，然后再写一篇徒手 dm-verity 实践的文章，结果偶然发现了本篇的英文原版。写得太好，我觉得自己完全没有必要再写原理了。

由于原文是英文，有些英语不太好的朋友不一定能完全理解。我花了点时间，结合 AI 将英语原文翻译成中文。



以下是翻译后的内容，对原文感兴趣的朋友，强烈建议去看看。

十分感谢原文作者 Ben Fogle 的付出~

# 嵌入式设备安全中的 dm-verity 简介
...
![img](./images-20250109-Android AVB 分析（十二）嵌入式设备安全中的 DM-VERITY 简介/Dm-verity-title.png)
...
![1736398124852](./images-20250109-Android AVB 分析（十二）嵌入式设备安全中的 DM-VERITY 简介/01-disk-hash-block.png)
...
![1736402876173](./images-20250109-Android AVB 分析（十二）嵌入式设备安全中的 DM-VERITY 简介/02-disk-hash-level.png)
...
![1736402916402](./images-20250109-Android AVB 分析（十二）嵌入式设备安全中的 DM-VERITY 简介/03-disk-hash-example.png)
...
上面看起来并没有发生什么特别的事情，那么引擎内部到底发生了什么？让我们从 `cat` 命令读取 one.txt 时会发生什么开始：



![1736402962956](./images-20250109-Android AVB 分析（十二）嵌入式设备安全中的 DM-VERITY 简介/04-cat-file1.png)
...
![1736402994380](./images-20250109-Android AVB 分析（十二）嵌入式设备安全中的 DM-VERITY 简介/05-cat-file1-2nd-time.png)
...
![1736403018866](./images-20250109-Android AVB 分析（十二）嵌入式设备安全中的 DM-VERITY 简介/06-cat-file2.png)
...
因为 dm-verity 在二进制级别保护磁盘，所以磁盘上任何位置的任一比特(bit)变化都会导致 dm-verity 引发错误。事实上，由于写入磁盘的元数据发生微小变化，仅对磁盘进行读写操作就足以让 dm-verity 变得崩溃。为了演示，


首先，让我们卸载之前的 dm-verity 设备：
